...
- `ask_user_question` 只有在宿主能隔离双方答案时才可称为“分别私密作答”；若同一会话会展示选择记录，让双方改用各自独立会话或受控私密表单，不能伪装成答案已隔离；
- 任一方不同意、被强迫或答案由另一方代填，停止。

## 3. 执行 SOP

1. 分别向 A、B 展示 `relationship_items.json` 中同一版本题目。有可隔离答案的 `ask_user_question@v1.0` 或等效工具时，必须分批使用可点击的 1—5 单选题卡；没有隔离能力时改用两个独立会话或私密表单，不在共同聊天里收集并展示逐题答案。
2. 生成输入：
