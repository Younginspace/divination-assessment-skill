...
7. **解释并校验**
   ...
   - 可继续展示 `Fi → Ne → Si → Te` 等常见类型学模型下的进一步偏好线索，但必须使用脚本返回的 `derived_function_preferences`，并紧邻说明“由四字母偏好代号推导，未单独测量”。
...
8. **可选生成分享卡**
   ...
   - 用户同意且宿主提供 `image_generation@v1.0` 时，按 [share-card.md](references/share-card.md) 只传入允许公开的结果摘要和视觉指令。
   - 默认生成 3:4 竖版；用户明确要求时可改为 1:1。图片生成后核对四字母代码、牌名、四柱、星座/月宿和免责声明；关键文字错误时只重试一次，仍错误就不交付图片，改给可复制的卡片文案。
...
- 分享卡是否由已校验 final 派生、完成脱敏与文字核对，并避免官方 MBTI®、预测和匹配率误导？
