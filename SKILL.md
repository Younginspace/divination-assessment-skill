...
- `ask_user_question@v1.0`：展示可点击的单选、多选、量表与结构化确认题；人格和关系问卷不得用聊天文字模拟弹窗。
- `write@v1.0`：只在本次私有运行目录新建答案、输入和 final JSON；不把敏感输入写入 Skill 安装目录。
- `read@v1.0`：读取参考文件、用户主动上传的报告、脚本结果和已校验的 SVG；不要越权读取无关文件。
- `bash@v1.0`：只运行随包 Python 脚本、校验器和脚本返回的精确 cleanup 命令；不联网安装依赖，不用 shell 重新实现计分或排盘。
- `image_generation/generate@v1.0`：只在用户明确同意更强插画感分享卡时生成无字背景或受限牌面预览；不能改写结果文字、分数、牌名或免责声明。

`edit@v1.0` 和 `load_skill@v1.0` 不是本 Skill 的运行依赖；Skill 由平台加载，新文件优先用 `write` 创建。若宿主未绑定 `ask_user_question`，只能明确退回文字答题，不能声称已展示交互弹窗。
...
4. **补齐最少输入**
   ...
   - 每轮合并询问 1—4 个必要问题。宿主提供 `ask_user_question@v1.0` 或等效结构化问答能力时，凡单选、多选、量表和日期/精度确认都必须调用它；不要把整套题目塞进普通聊天气泡，也不要要求用户粘贴 `A、B、B…` 或数字数组。只有工具不存在、调用失败或题型不受支持时，才退回编号文字题，并用一句话说明。
   - 工具每次可展示的题数以宿主 schema 上限为准，分批收齐后按题目 id 规范化保存；不得因分批、重试或按钮换位改变计分映射。工具返回前不要假装用户已经作答，也不要用合成答案代填。
...
8. **可选生成分享卡**
   ...
   - 用户明确想要更强插画感且宿主提供 `image_generation/generate@v1.0` 时，运行 `build_share_card_image_prompt.py`：生产默认使用 `text-free-background`，再由宿主叠加已校验 SVG；用户明确要“像一张完整塔罗牌”的预览时可传 `--render-pass framed-preview`，只生成牌号、双语牌名和正逆位关键词。逐字核对预览文字，错误时回退到无字背景加确定性文字层。
...
- 宿主有 `ask_user_question` 时，问卷是否真实显示了可点击题卡，而不是在聊天中省略答题或让用户粘贴答案串？
- 用户可见对话是否像真实聊天：先回应具体处境、短句推进、结果先说人话，并把原始答案数组、hash、错误码和内部字段留在证据层？
- 是否使用唯一私有运行目录并在结束时清理？
